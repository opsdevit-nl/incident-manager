<script>
    ...
    // Date filter handler – note that we pass both the key and the event:
      ...
      <div class="d-flex w-100 justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center">
          ...
          <button on:click={() => toggleDarkMode()} 
            ...
            {#if darkMode} <i class="bi bi-sun-fill"></i> {:else} <i class="bi bi-sun"></i> {/if}
          </button>
          <button on:click={() => logout()} class="btn btn-secondary mr-2">Logout</button>
          <!-- Overlay trigger button -->
          <button class="btn btn-outline-light btn-sm" on:click={() => showFiltersModal = true}>
            Filters &amp; Sorting
          </button>
        </div>
      </div>
